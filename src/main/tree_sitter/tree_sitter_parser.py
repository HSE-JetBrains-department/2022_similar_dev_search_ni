from src.main.dulwich.extract import clone_repo
...
def bfs_traverse_tree(root: Node) -> List[Node]:
    """
    Iterative BFS method for traversing parse tree
    given by tree-sitter library.
...
def iterate_nodes(trees_nodes_list: List[Node]):
    for node in trees_nodes_list:
        print(node.type)
        # if(node.type == 'identifier'): #identifier, import_statement
